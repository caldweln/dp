...
function NotMnist:__init(config)
   ...
   self._args, self._fracture_data, self._valid_ratio, self._train_dir, self._test_dir,
...
function NotMnist:loadTrainValid()
   ...
   local dataSize = math.floor(inputs:size(1) * self._fracture_data)
   -- train
   local start = 1
...
function NotMnist:loadTest()
   ...
   local dataSize = math.floor(inputs:size(1) * self._fracture_data)
   self:testSet(self:createDataSet(inputs:narrow(1,1,dataSize), targets:narrow(1,1,dataSize), 'test'))
   return self:testSet()
end
